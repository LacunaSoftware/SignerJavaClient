...
public class SignerClient {
	...
	public DocumentModel getDocumentDetails(UUID id) throws RestException {
		LacunaSignerApiDocumentsDocumentModel model = getRestClient()
				.get(String.format("/api/documents/%s", id.toString()), LacunaSignerApiDocumentsDocumentModel.class);
	...
	public TicketModel getDocumentDownloadTicket(UUID id, DocumentTicketType type) throws RestException {
		LacunaSignerApiTicketModel model = getRestClient().get(
				String.format("/api/documents/%s/ticket?type=%s", id.toString(), type.getValue()),
				LacunaSignerApiTicketModel.class);
	...
	public StartSignatureResponse startPublicSignature(String key, StartSignatureRequest request) throws RestException {
		LacunaSignerApiSignatureStartSignatureResponse response = getRestClient().post(
				String.format("/api/documents/keys/%s/signature/certificate", key), request.toModel(),
				LacunaSignerApiSignatureStartSignatureResponse.class);
		return new StartSignatureResponse(response);
	}

	public CompleteSignatureResponse completePublicSignature(String key, CompleteSignatureRequest request)
			throws RestException {
		LacunaSignerApiSignatureCompleteSignatureResponse response = getRestClient().post(
				String.format("/api/documents/keys/%s/signature", key), request.toModel(),
				LacunaSignerApiSignatureCompleteSignatureResponse.class);
	...
	public StartSignatureResponse startSignature(UUID id, StartSignatureRequest request) throws RestException {
		LacunaSignerApiSignatureStartSignatureResponse response = getRestClient().post(
				String.format("/api/documents/%s/signature/certificate", id.toString()), request.toModel(),
				LacunaSignerApiSignatureStartSignatureResponse.class);
		return new StartSignatureResponse(response);
	}
	...
	public CompleteSignatureResponse completeSignature(UUID id, CompleteSignatureRequest request) throws RestException {
		LacunaSignerApiSignatureCompleteSignatureResponse response = getRestClient().post(
				String.format("/api/documents/%s/signature", id.toString()), request.toModel(),
				LacunaSignerApiSignatureCompleteSignatureResponse.class);
		return new CompleteSignatureResponse(response);
	}

	public void sendElectronicSignatureAuthenticationCode(SendElectronicSignatureAuthenticationRequest request)
			throws RestException {
	...
	public FolderInfoModel createFolder(FolderCreateRequest request) throws RestException {
		LacunaSignerApiFoldersFolderInfoModel model = getRestClient().post("/api/folders", request.toModel(),
				LacunaSignerApiFoldersFolderInfoModel.class);
		return new FolderInfoModel(model);
	}

	public FolderDetailsModel getFolderDetails(UUID folderId) throws RestException {
		LacunaSignerApiFoldersFolderDetailsModel model = getRestClient().get(
				String.format("/api/folders/%s/details", folderId.toString()),
				LacunaSignerApiFoldersFolderDetailsModel.class);
		return new FolderDetailsModel(model);
	}

	@SuppressWarnings("unchecked")
	public PaginatedSearchResponse<FolderInfoModel> listFoldersPaginated(PaginatedSearchParams searchParams,
			UUID organizationId) throws RestException {
		String orgIdStr = organizationId != null ? organizationId.toString() : "";
		LacunaSignerApiPaginatedSearchResponse<LacunaSignerApiFoldersFolderInfoModel> model = (LacunaSignerApiPaginatedSearchResponse<LacunaSignerApiFoldersFolderInfoModel>) getRestClient()
				.get(String.format("/api/folders%s&organizationId=%s", buildSearchPaginatedParamsString(searchParams),
						orgIdStr),
						TypeToken.getParameterized(LacunaSignerApiPaginatedSearchResponse.class,
								LacunaSignerApiFoldersFolderInfoModel.class));
	...
	private String buildSearchPaginatedParamsString(PaginatedSearchParams searchParams) {
		return String.format("?q=%s&limit=%s&offset=%s", getParameterOrEmpty(searchParams.getQ()),
				searchParams.getLimit(), searchParams.getOffset());
	}
